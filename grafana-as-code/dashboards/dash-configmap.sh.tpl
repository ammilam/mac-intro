...
cu() {
TMPFILE=$(mktemp)
curl "IP/api/search"|jq -r '.' > $TMPFILE
...
name() {
   ...
   curl "IP/api/dashboards/uid/$ID/"|jq -r '.dashboard|del(.meta)|.id = null' > $TMPFILE2
   kubectl create configmap $NAME --namespace=-prometheus --from-file=$TMPFILE2 --dry-run=true -o yaml|yq w - --tag '!!str' metadata.labels.grafana_dashboard '1' |grep -v creationTimestamp > "${NAME}.yaml"
